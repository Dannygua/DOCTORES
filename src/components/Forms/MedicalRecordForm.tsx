...
const MedicalRecordForm = ({
  ...
  const setSomeValues = (date: any) => {
    ...
    if (date.length > 0) {
      ...
      if ('record' in date[0]) {
        form.setFieldValue("esAptoOperation", date[0].record.generalInfo.esAptoOperation || "")
  ...
  const onFinish = async (values: any) => {
    ...
    let associatedTestsApi = [];
    if (values.associatedTests) {
      associatedTestsApi = values.associatedTests.map((associatedTests: any) => {
        return {
          name: typeof associatedTests === "string" ? associatedTests : associatedTests.value,
        };
      });
    }
    ...
    if (values.testResults) {
      ...
      console.log('treee', values.testResults)
    ...
    const completeValues = {
      ...
      esAptoOperation: values.esAptoOperation,
      ...
      associatedTests: associatedTestsApi,
    ...
    console.log("valores que se mandan del form", completeValues)
    ...
    if ("record" in date) {
      ...
      console.log("request creaci", request)
      if (request.status) {
  ...
  const getImcMessage = () => {
    ...
    if (parsedValue > 35 && parsedValue <= 39.9) {
      dinamicText = "Obesidad severa, apto para la cirugía"
      return baseText + dinamicText;
    }
            ...
            <Row>
              <Col xxl={11} xl={11} lg={11} md={11} sm={24} xs={24}>
                ...
                <Form.Item 
                  ...
                  name="esAptoOperation"
                  rules={[
                    {
                      required: true,
                      message: "Por favor ingrese si es apto o no",
                      type: 'boolean',  
                    },
                  ]}
                  valuePropName="value"  
                ...
                <Row>
                  ...
                  <Col span={16}>
                    <Form.Item
                      ...
                      extra="Ingrese el nombre y siga el orden en que fueron cargados los resultados"
                    >
                      {tests && tests.length > 0 && (
                        <Select
